...
const CountryDetails = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetchCountry(authorization, ccn3)
          ...
          country ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                ...
                <h2 className="countries-title mb-0">Detalhes do País</h2>
              </div>
              {/* Seção de comentários reais */}
              {country.ccn3 && /^\d{3}$/.test(country.ccn3) && (
                <CountryComments countryCode={country.ccn3} />
              )}
            ...
            <div className="alert alert-warning mt-5">País não encontrado.</div>
          )
        )}
      </div>
    </main>
